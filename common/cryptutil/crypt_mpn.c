...
static mp_bitcnt_t mpn_common_scan (mp_limb_t limb, mp_size_t i, mp_srcptr up, mp_size_t un,
		 mp_limb_t ux)
{
  unsigned cnt;

  ASSERT_EXPR (ux == 0 || ux == GMP_LIMB_MAX);
  ASSERT_EXPR (0 <= i && i <= un );
...
mpn_invert_3by2 (mp_limb_t u1, mp_limb_t u0)
{
   ...
   * B^2 - (B + m) u1 <= u1 */
  ASSERT_EXPR (u1 >= GMP_LIMB_HIGHBIT);
...
static void mpn_div_qr_2_preinv (mp_ptr qp, mp_ptr rp, mp_srcptr np, mp_size_t nn,
		     const struct gmp_div_inverse *inv)
{
  unsigned shift;
  mp_size_t i;
  ...
  if (shift > 0)
    {
      ASSERT_EXPR ((r0 << (GMP_LIMB_BITS - shift)) == 0);
...
#if 0
static void mpn_div_qr_2 (mp_ptr qp, mp_ptr rp, mp_srcptr np, mp_size_t nn,
	      mp_limb_t d1, mp_limb_t d0)
{
  struct gmp_div_inverse inv;
  ASSERT_EXPR (nn >= 2);
		...
		mp_limb_t dinv)
{
  ...
  ASSERT_EXPR ((d1 & GMP_LIMB_HIGHBIT) != 0);
		   ...
		   const struct gmp_div_inverse *inv)
{
  ...
  if (dn == 1)
  ...
  else
    {
      ...
      ASSERT_EXPR (inv->d1 == dp[dn-1]);
      ASSERT_EXPR (inv->d0 == dp[dn-2]);
      ASSERT_EXPR ((inv->d1 & GMP_LIMB_HIGHBIT) != 0);
  ...
  for (i = 0; w > 0; i++)
    {
      ...
      ASSERT_EXPR ( (r << (GMP_LIMB_BITS - binv->shift)) == 0);
      r >>= binv->shift;
...
static mp_size_t mpn_set_str_other (mp_ptr rp, const unsigned char *sp, size_t sn,
...
static void * mpz_realloc (mpz_t r, mp_size_t size)
{
  size = GMP_MAX (size, 1);
...
static int mpz_div_qr (mpz_t q, mpz_t r,
...
static unsigned long mpz_div_qr_ui (mpz_t q, mpz_t r,
...
mpz_rootrem (mpz_t x, mpz_t r, const mpz_t y, unsigned long z)
{
  ...
  {
    mp_bitcnt_t tb;
    tb = (mp_bitcnt_t)(mpz_sizeinbase (y, 2) / z + 1);
    mpz_init2 (t, tb);
    mpz_setbit (t, tb);
  }
...
mpz_set_str (mpz_t r, const char *sp, int base)
{
  ...
  ASSERT_EXPR (base == 0 || (base >= 2 && base <= 36));
  ...
  if (bits > 0)
    {
      alloc = (mp_size_t)((sn * bits + GMP_LIMB_BITS - 1) / GMP_LIMB_BITS);
  ...
  else
    {
      struct mpn_base_info info;
      mpn_get_base_info (&info, base);
      alloc = (mp_size_t)((sn + info.exp - 1) / info.exp);
      rp = MPZ_REALLOC (r, alloc);
      rn = mpn_set_str_other (rp, dp, sn, base, &info);
    }
  ASSERT_EXPR (rn <= alloc);
  gmp_free (dp);
...
//    gmp_die ("mpz_import: Nails not supported.");
  ...
  ASSERT_EXPR (order == 1 || order == -1);
  ASSERT_EXPR (endian >= -1 && endian <= 1);
...
//    gmp_die ("mpz_import: Nails not supported.");
  ...
  ASSERT_EXPR (order == 1 || order == -1);
  ASSERT_EXPR (endian >= -1 && endian <= 1);
  ASSERT_EXPR (size > 0 || u->_mp_size == 0);
