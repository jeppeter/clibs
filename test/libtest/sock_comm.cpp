...
sock_comm::sock_comm(void* psock)
{
	this->m_sock = psock;
	this->m_prdbuf = NULL;
	this->m_rdlen = 0;
...
{
	if (this->m_sock !=NULL) {
		DEBUG_INFO("close [%s]:[%s]", tcp_sock_sockname(this->m_sock),tcp_sock_peername(this->m_sock));
	}
...
int sock_comm::is_read_mode()
{
	int ret =0;
	HANDLE hret;
	if (this->m_inited != 0) {
		hret = get_tcp_read_handle(this->m_sock);
		if (hret != NULL) {
			ret = 1;
		}
	}
...
int sock_comm::is_write_mode()
{
	int ret =0;
	HANDLE hret;
	if (this->m_inited != 0) {
		hret = get_tcp_write_handle(this->m_sock);
		if (hret != NULL) {
			ret = 1;
		}
	}
...
HANDLE sock_comm::get_write_evt()
{
	HANDLE hret = NULL;
	if (this->m_inited != 0) {
		hret = get_tcp_write_handle(this->m_sock);
...
int sock_comm::__inner_read()
{
	...
	if (this->m_rdlen < PROTO_HDR_SIZE) {
		...
		ret = read_tcp_socket(this->m_sock,(uint8_t*)&(this->m_prdbuf[this->m_rdlen]),(this->m_needlen - this->m_rdlen));
		if (ret < 0) {
			GETERRNO(ret);
			SETERRNO(ret);
			return ret;
...
int sock_comm::complete_read()
{
	...
	ret = complete_tcp_read(this->m_sock);
	if (ret < 0) {
		GETERRNO(ret);
		SETERRNO(ret);
		return ret;
...
int sock_comm::read_json(jvalue** ppj)
{
	int ret;
	int rdlen;
...
write_again:
	...
	ret = write_tcp_socket(this->m_sock,(uint8_t*)this->m_pwrbuf,this->m_wrlen);
	if (ret < 0) {
		GETERRNO(ret);
		SETERRNO(ret);
		return ret;
...
int sock_comm::write_json(jvalue* pj)
{
	int ret;
	char* jsonstr=NULL;
	unsigned int jsonsize=0;
...
int sock_comm::complete_write()
{
	...
	ret = complete_tcp_write(this->m_sock);
	if (ret < 0) {
		GETERRNO(ret);
		SETERRNO(ret);
		return ret;
