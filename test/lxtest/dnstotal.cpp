...
int DnsTotal::__get_result(DnsCap* pcap)
{
	...
	while(1) {
		...
		if (vstr.compare(0,6,"ERROR;") == 0) {
			...
			std::map<std::string,std::vector<std::string> >::iterator iter = this->m_ipres.find(k);
			...
			if (iter != this->m_ipres.end()) {
				...
				this->m_ipres.insert(std::make_pair(k,vvec));
			}
		}		
	}
	return cnt;
...
int DnsTotal::get_result(std::map<std::string,std::vector<std::string> >& res)
{
	int cnt=0;
	res.clear();
	for(std::map<std::string,std::vector<std::string> >::iterator iter = this->m_ipres.begin();iter != this->m_ipres.end();++ iter) {
