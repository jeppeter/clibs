...
int runsingle_handler(int argc, char* argv[], pextargs_state_t parsestate, void* popt)
{
    ...
    for(i=0;parsestate->leftargs && parsestate->leftargs[i];i++) {
        if (i > 0) {
            ret = append_snprintf_safe(&cmdstr,&cmdsize," %s", parsestate->leftargs[i]);
        } else {
            ret = snprintf_safe(&cmdstr,&cmdsize,"%s", parsestate->leftargs[i]);
        }
        if (ret < 0) {
            GETERRNO(ret);
            goto out;
    ...
    ret = run_cmd_event_output_single(st_evtfd, pin, inlen, ppout, &outsize, pperr, &errsize, &exitcode, pargs->m_timeout, cmdstr);
    if (ret < 0) {
        GETERRNO(ret);
        fprintf(stderr, "run command error [%d]\n", ret);
        goto out;
    ...
    fprintf(stdout, "run command [%s] exitcode [%d]\n",cmdstr,exitcode);
    if (pargs->m_input != NULL) {
        fprintf(stdout, "input out\n");
        __debug_buf(stdout, pin, inlen);
    } else {
...
out:
    run_cmd_event_outputv(-1, NULL, 0, &pout, &outsize, &perr, &errsize, NULL, 0, NULL);
    snprintf_safe(&cmdstr,&cmdsize,NULL);
    read_file_whole(NULL, &pin, &insize);
    if (st_evtfd >= 0) {
        close(st_evtfd);
    }
